...
//changes 10-13-22 1:00AM 
//vocab speed changed
//Removed note reading, aslap,
//removed a few minor bugs note some will still be present
//movable window still un the works
//stealth error will be patched but supported browsers Edge and Chrome
...
setTimeout( //2 sec delay to load before trying to run
    ...
    function init() {
        ...
        //menu buttons
        buildMenuButton("Tweaks Menu", "tweaksbutton", () => {
        ...
        buildMenuButton("Search Selection", "googlebutton", () => {
            var result = window.getSelection().toString();
            window.openedWindows.forEach((window) => {
                if (window != null) {
                    window.close();
                }
                window = null;
            ...
            if (result != "") {
                ...
                if ($("#brainlysearch").is(":checked"))
                    window.openedWindows[1] = window.open("https://brainly.com/app/ask?q=" + result);
                if ($("#wolframsearch").is(":checked"))
                    window.openedWindows[2] = window.open("https://www.wolframalpha.com/input/?i=" + result);
                if ($("#customsearch").is(":checked"))
                    window.openedWindows[3] = window.open("https://google.com/search?q=site:" + $("#css").val() + " " + result);
                localStorage.setItem("csskey", $("#css").val());
            }

            else
                $("#userconsole").prepend("<li>There's nothing selected! ");
        });
        buildMenuButton("Search Config", "scbutton", () => {
        ...
        $("#googlebutton").on("mouseenter", () => {
            $("#scbutton").fadeIn();
        }); //Hide / Show search config
        $("#googlebutton").on('mouseleave', () => {
            $("#scbutton").fadeOut();
        });
        $("#scbutton").hide();
        buildMenuButton("Guess this", "guessbutton", () => {
            try {
        ...
        buildMenuButton("Toggle Console", "consolebutton", () => {
            $("#consolediv").toggle();
        });
    ...
    function loop() {
        ...
        if ($("#AutoAdvance").is(":checked"))
            autoadvance();
        if ($("#SkipIntro").is(":checked"))
            skipIntro();
        if ($("#GuessPractice").is(":checked"))
            GuessPractice();
